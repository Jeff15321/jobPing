...
func main() {
	...
	queueURL := os.Getenv("USER_ANALYSIS_QUEUE_URL")
	if queueURL == "" {
		log.Fatal("USER_ANALYSIS_QUEUE_URL environment variable is required")
	}
	...
	// Start polling
	log.Printf("Starting user analysis worker for queue: %s", queueURL)
	if err := poller.Start(ctx); err != nil && err != context.Canceled {
		log.Fatalf("Poller error: %v", err)
	}

	log.Println("User analysis worker stopped")
}
