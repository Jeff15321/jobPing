...
func main() {
	...
	queueURL := os.Getenv("JOB_ANALYSIS_QUEUE_URL")
	if queueURL == "" {
		log.Fatal("JOB_ANALYSIS_QUEUE_URL environment variable is required")
	}

	// Build application
	appInstance, err := app.BuildJobAnalysis()
	if err != nil {
	...
	// Start polling
	log.Printf("Starting job analysis worker for queue: %s", queueURL)
	if err := poller.Start(ctx); err != nil && err != context.Canceled {
		log.Fatalf("Poller error: %v", err)
	}

	log.Println("Job analysis worker stopped")
}
